...
describe("CurrencyConverterTransactionRepository", () => {
  ...
  describe("insertCurrencyConverterTransaction", () => {
    it("should call repository.insert with correct data", async () => {
      ...
      expect(mockDataSource.getRepository).toHaveBeenCalledWith(CurrencyConverterTransactionEntity);
    });
    ...
    it("should throw error when repository.insert fails", async () => {
      ...
      await expect(repository.insertCurrencyConverterTransaction(mockTransactionData)).rejects.toThrow(errorMessage);
    });
    ...
    it("should propagate TypeORM specific errors", async () => {
      ...
      await expect(repository.insertCurrencyConverterTransaction(mockTransactionData)).rejects.toThrow("SQL error: duplicate key");
    });
